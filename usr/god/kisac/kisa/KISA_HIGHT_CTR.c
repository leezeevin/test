...
int HIGHT_CTR_Process( OUT KISA_HIGHT_INFO *pInfo, IN DWORD *in, IN int inLen, OUT DWORD *out, OUT int *outLen )
{
  ...
  printf(" inLen : %d , *outLen : %d, nCurrentCount : %d \r\n", inLen, *outLen, nCurrentCount);
...
void main_kisa_hight_ctr()
{
  ...
  for (i=0;i<26;i++) *(InputBytes+i) = 'A'+i;
  for (i=0;i<26;i++) *(InputBytes+i+26) = 'a'+i;
  for (i=0;i<10;i++) *(InputBytes+i+52) = '0'+i;
  ...
  InputCount[0] = nInputTextLen;
	...
	else
	{
      ...
      PRINT_BUFFER("outbuf 1", outbuf, nOutLeng);
    ...
    PRINT_BUFFER("outbuf 2", outbuf, nOutLeng);
		HIGHT_CTR_Close( &info, outbuf, nOutLeng, &nEncRmainLeng );
    PRINT_BUFFER("outbuf 3", outbuf, nEncRmainLeng);
		cdata = int32tochar_for_HIGHT_CTR(outbuf, nOutLeng - nEncRmainLeng);
