...
int HIGHT_CBC_Process( OUT KISA_HIGHT_INFO *pInfo, IN DWORD *in, IN int inLen, OUT DWORD *out, OUT int *outLen )
{
	...
	if( KISA_ENCRYPT == pInfo->encrypt )
	{
		...
		*outLen = nCurrentCount - BLOCK_SIZE_HIGHT;
		...
		PRINT_BUFFER("pInfo->ivec", pInfo->ivec, 8);
	}
	else
	{
		pdwXOR = pInfo->ivec;
...
int HIGHT_CBC_Close( OUT KISA_HIGHT_INFO *pInfo, IN DWORD *out, IN int out_offset, IN int *outLen )
{
	...
	*outLen = 0;
	...
	if( KISA_ENCRYPT == pInfo->encrypt )
	{
		...
		PRINT_BUFFER(">>>> pInfo->cbc_buffer", pInfo->cbc_buffer, pInfo->buffer_length);
		PRINT_BUFFER(">>>> pOut", pOut, pInfo->buffer_length);
...
// method 2 start
void main_kisa_hight_cbc()
{

	BYTE pbszUserKey[16] = {0,}; 
	BYTE pbszIV[16] = {0, };
	BYTE plainText[71] = {0,};
	BYTE pbszCipherText[80] = {0x00};
	BYTE pbszPlainText[80] = {0x00};
  ...
  //for (i=0;i<26;i++) *(PlainBytes+i) = 'A'+i;
  //for (i=0;i<26;i++) *(PlainBytes+i+26) = 'a'+i;
  //for (i=0;i<10;i++) *(PlainBytes+i+52) = '0'+i;



	ascii_to_hex("828E9E067BC2E9B306A3FA99426787AC", pbszUserKey);
	ascii_to_hex("21C7B0986CF8265708DD9202777B35E7", pbszIV);
	ascii_to_hex("A1ADCBDDD519B6D40B6258B06CA0C158", plainText);
	...
	HIGHT_CBC_init( &info, KISA_ENCRYPT, pbszUserKey, pbszIV );
	PRINT_BUFFER("hightkey", info.hight_key.key_data, 128);
	...
	for (i = 0; i < message_length - process_blockLeng; )
	{
		...
		PRINT_BUFFER("1 pbszIV", pbszIV, 8);
	...
	PRINT_BUFFER("2 pbszIV", pbszIV, 8);
	HIGHT_CBC_Process( &info, data, remainleng, outbuf, &nRetOutLeng );
	PRINT_BUFFER("3 pbszIV", pbszIV, 8);
	cdata = int32tochar_for_HIGHT_CBC( outbuf, nRetOutLeng);
	memcpy( CipherBytes + i, cdata, nRetOutLeng );
	i += nRetOutLeng;
	free(cdata);
	...
	else
	{
		...
		for (i = 0; i < EncryptedMessage_length - process_blockLeng; )
		{
			...
			HIGHT_CBC_Process( &info, data, process_blockLeng, outbuf, &nRetOutLeng );
      PRINT_BUFFER("outbuf 1 ", outbuf, nRetOutLeng);
			cdata = int32tochar_for_HIGHT_CBC( outbuf, nRetOutLeng);
