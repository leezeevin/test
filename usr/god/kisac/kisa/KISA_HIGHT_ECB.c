...
{
	BYTE pdwRoundKey[1024] = {0,};																									// Round keys for encryption or decryption
	BYTE pbUserKey[1024] = {0x88, 0xE3, 0x4F, 0x8F, 0x08, 0x17, 0x79, 0xF1, 0xE9, 0xF3, 0x94, 0x37, 0x0A, 0xD4, 0x05, 0x89 }; 		// User secret key
	BYTE pbData[1024]    = {0xD7, 0x6D, 0x0D,0x18, 0x32, 0x7E, 0xC5, 0x62}; 															// input plaintext to be encrypted
	BYTE pbCT[1024]    = {0xD7, 0x6D, 0x0D,0x18, 0x32, 0x7E, 0xC5, 0x62}; 															// input plaintext to be encrypted

	int i;
  int e;

  e  = ascii_to_hex(KISA_KEY_HIGHT_ECB[0][0], pbUserKey);
  //display("USER KEY", pbUserKey, e);


  e  = ascii_to_hex(KISA_KEY_HIGHT_ECB[0][1], pbData);
  //display("PT", pbData, e);

  e  = ascii_to_hex(KISA_KEY_HIGHT_ECB[0][2], pbCT);
  //display("CT", pbCT, e);
...
// Derive roundkeys from user secret key
  ...
  //display("pdwRoundKey", pdwRoundKey, 16);
