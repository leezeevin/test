...
void* WebSocketStatus(void* t,int fd, unsigned char* b, int sz, void* mi, int err, void* o)
{
  int e = 0;
...
void* WebSocketRead(void* t,int fd, unsigned char* b, int sz, void* mi, int err, void* o)
{
  int e = 0;
  char st[32];
  char rbuf[4096];
...
void* SocketStatus(void* t,int fd, unsigned char* b, int sz, void* mi, int err, void* o)
{
  int e = 0;
  int idx = (mi==0)?0:(*(int*)mi);
...
void* SocketRead(void* t,int fd, unsigned char* b, int sz, void* mi, int err, void* o)
{
  char st[32];
  char rbuf[1024];
  char vbuf[1024];
 ...
 * Serial
 *******************************************************************************/
void* SerialStatus(void* t,void* fd, unsigned char* b,int sz,void* mi, int err, void* o)
{
  int e = 0;
...
void* SerialRead(void* t,void* fd, unsigned char* b,int sz,void* mi, int err, void* o)
{
  char st[32];
  char rbuf[1024];
  char vbuf[1024];
  ...
  int w = 0;
  if ( gp->read_lock ) return 0;
  if ( err < 0 ) return SerialStatus(t,fd,b,sz,mi,err,o);
...
/*******************************************************************************/
void* SerialStatus2(void* t,void* fd, unsigned char* b,int sz,void* mi, int err, void* o)
{
  int e = 0;
  int r = 0;
  return e;
...
void* SerialRead2(void* t,void* fd, unsigned char* b,int sz,void* mi, int err, void* o)
{
  char st[32];
  char rbuf[1024];
  char vbuf[1024];
  ...
  int w = 0;
  if ( gp->read_lock ) return 0;
  if ( err < 0 ) return SerialStatus2(t,fd,b,sz,mi,err,o);
  *(b+sz) = 0;
...
void* wmtif_IDC_BUTTON_NET_OPEN_CLOSE(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
  ...
  uint8_t temp[32] = {0};
  int32_t e = 0;
  SetText(gp->item[_IDC_EDIT_NET_WRITE].hwnd, ("000100000006000300000008"), strlen("000100000006000300000008"));
