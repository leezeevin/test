...
int SocketOpen()
{
  GetText(gp->hitem[EDIT_NET_IP], ((mtifInfo*)gp->pm->msck.info)->arg[XSOCKET][XM_IP], 32);
  GetText(gp->hitem[EDIT_NET_PORT], ((mtifInfo*)gp->pm->msck.info)->arg[XSOCKET][XM_PORT], 32);

  GetComboText(gp->hitem[COMBO_NET_PROTOCOL], ((mtifInfo*)gp->pm->msck.info)->arg[XSOCKET][XM_PROTO]);
  GetComboText(gp->hitem[COMBO_NET_CS],       ((mtifInfo*)gp->pm->msck.info)->arg[XSOCKET][XM_CSTYPE]);
  GetComboText(gp->hitem[COMBO_NET_CASTTYPE], ((mtifInfo*)gp->pm->msck.info)->arg[XSOCKET][XM_CASTTYPE]);

  lock_init((void*)&gp->pm->msck.crit);

  gp->pm->msck.m.pfxMTIF[ONSOCKETSTATUS] = SocketStatus;
  gp->pm->msck.m.pfxMTIF[ONSOCKETREAD] = SocketRead;

  gp->pm->msck.b[0][0] =  0x80|SOCKET_BUF_SFT;
  gp->pm->msck.fd = mtifSocketOpen(&gp->pm->msck.m,
                          ...
                          ((mtifInfo*)gp->pm->msck.info)->arg[XSOCKET][XM_CASTTYPE], gp->pm->msck.b[0], (void*)&gp->pm->msck);
