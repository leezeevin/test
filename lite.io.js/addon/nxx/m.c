...
typedef struct
{
  ...
  napi_threadsafe_function tsfx; //0 to JS, 1 to Native
  struct nArg arg;
...
static napi_value liteio_write(napi_env env, napi_callback_info info)
{
  ...
  {
    if ( napi_get_buffer_info(env, args[1],(void**)&parg->b, &_asz) != napi_ok )
    {
      free(parg);
      return 0;
    }
    parg->asz = parg->sz;
  	DEBUG_PRINT("napi_object %4d:%4d, [%s]\r\n", _asz, parg->asz, parg->b);
  }
  else if ( vtype[idx] == napi_string )
  {
    parg->bs = (int8_t*)calloc(parg->sz*4, sizeof(int8_t));
    if ( napi_get_value_string_utf8(env, args[1], parg->bs, parg->sz*4, &parg->asz) != napi_ok )
    {
      free(parg->bs);
      free(parg);
      return 0;
  ...
  {
    idx = 3;
    if ( napi_get_value_string_utf8(env, args[3], parg->ext, MAX_BUF_SZ, &_asz) != napi_ok )
    {
      free(parg->ext);
...
#if 0
static void js_call(napi_env env, napi_value js_cb, void* context, void* arg)
{
  // This parameter is not used.
  (void) context;
  napi_value undefined;
...
static void js_call(napi_env env, napi_value js_cb, void* context, void* arg)
{
  ...
  if (env != 0)
  {
    ...
    NAPI_STATUS(napi_create_uint32(env, _nliteio->arg.ecode, &_nv[idx]));
    idx++;

    NAPI_STATUS(napi_get_undefined(env, &undefined));

    DEBUG_PRINT(" napi_call_function -> %d, %d, %d(%08X)\r\n"
                "////////////////////////////////////////////////////////////////////////////////\r\n"
                "%s\r\n"
                "################################################################################\r\n",
                idx, _nliteio->arg.sz, _nliteio->arg.ecode, _nliteio->arg.ecode, _nliteio->arg.b);
...
static int32_t __ncallback_reader(void* obj, void* arg)
{
  ...
  // _nliteio->nidx++;
  // _nliteio->nidx %= MAX_NARG;
  int32_t e = __get_hex_semaphore(&_nliteio->SR, 0x00000001, 0x00000001, 4000000);
  DEBUG_PRINT(" e---> %d %08X\r\n",e, _nliteio->SR );
...
static napi_value liteio_open(napi_env env, napi_callback_info info)
{
  ...
  NAPI_STATUS(napi_create_threadsafe_function(env, args[1], 0, work_name,0,1,0,0,0,js_call, &_nliteio->tsfx)); // from callback
...
napi_value liteio_init(napi_env env, napi_value exports)
{
  ...
  #endif
  #if 1
  NAPI_STATUS(napi_create_function(env, 0, 0, liteio_open, _nliteio, &_fx[0]));
  NAPI_STATUS(napi_set_named_property(env, exports, "open", _fx[0]));

  NAPI_STATUS(napi_create_function(env, 0, 0, liteio_write, _nliteio, &_fx[1]));
  NAPI_STATUS(napi_set_named_property(env, exports, "write", _fx[1]));

  NAPI_STATUS(napi_create_function(env, 0, 0, liteio_read, _nliteio, &_fx[2]));
  NAPI_STATUS(napi_set_named_property(env, exports, "read", _fx[2]));

  NAPI_STATUS(napi_create_function(env, 0, 0, liteio_close, _nliteio, &_fx[3]));
  NAPI_STATUS(napi_set_named_property(env, exports, "close", _fx[3]));

  NAPI_STATUS(napi_create_function(env, 0, 0, liteio_debug, _nliteio, &_fx[4]));
  ...
  status = napi_define_properties(env, exports, sizeof(_workers)/sizeof(_workers[0]), _workers);
  assert(status == napi_ok);
  #endif
  return exports;
}
