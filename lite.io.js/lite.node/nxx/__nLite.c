...
#if __NWEBSOCKET__
int32_t __liteio_websocket_write(void* h, int32_t fd, int8_t* b, int32_t sz, void* extra, void* o)
{
  int32_t e = 0;
  struct _litePoint* liteP = (struct _litePoint*)h;
  if ( liteP == 0 ) return 0xEFFFFFFF;
  DEBUG_PRINT("----> %d %s \r\n", sz, b);
  e = liteP->__writer[NWEBSOCKET](&liteP->__websocket, fd, b, sz, extra, &liteP->__websocket);
  return e;
}
#endif

#if __NWEBSOCKET__
int32_t __liteio_websocket_close(void* h, int32_t fd, int8_t* b, int32_t sz, void* extra, void* o)
{
  int32_t e = 0;
  struct _litePoint* liteP = (struct _litePoint*)h;
  if ( liteP == 0 ) return 0xEFFFFFFF;

  if ( liteP->__websocket.io.fd == fd )
  {
    e = liteP->__closer[NWEBSOCKET](&liteP->__websocket, fd, b, sz, extra, &liteP->__websocket);
  }
  else
  {
    e = liteP->__closefd[NWEBSOCKET](&liteP->__websocket, fd, b, sz, extra, &liteP->__websocket);
  }
...
void* setup(void* arg)
{
  ...
  for ( ; c>0 ; c-- )
  {
    ...
    #if __NWEBSOCKET__
    if ( strcompare(pcmd, "websocket") == 0 )
    {
      liteP->__writer[NWEBSOCKET] = __nwebsocket_write;
      liteP->__closer[NWEBSOCKET] = __nwebsocket_close;
      liteP->__closefd[NWEBSOCKET] = __nwebsocket_close_fd;

      readArgs(liteP->argc, liteP->argv, "--websocket_url",            liteP->__websocket.io.argv[WEBSOCKET_URL]);
      readArgs(liteP->argc, liteP->argv, "--websocket_cstype",         liteP->__websocket.io.argv[WEBSOCKET_CSTYPE]);
      readArgs(liteP->argc, liteP->argv, "--websocket_size_of_buffer", liteP->__websocket.io.argv[SIZE_OF_BUFFER]);

      DEBUG_PRINT("%s, %s, %s\r\n",
            liteP->__websocket.io.argv[WEBSOCKET_URL],
					  liteP->__websocket.io.argv[WEBSOCKET_CSTYPE],
            liteP->__websocket.io.argv[SIZE_OF_BUFFER]);


      liteP->__websocket.io.__f[__CALLBACK_CONNECTED] = __liteio_call;
      liteP->__websocket.io.__f[__CALLBACK_DISCONNECTED] = __liteio_call;
      liteP->__websocket.io.__f[__CALLBACK_READ] = __liteio_call;
      liteP->__websocket.io.__o = liteP;

      e = __nwebsocket_open(&liteP->__websocket);
      DEBUG_PRINT(" __nwebsocket_open : %08X(%d) \r\n", e,e);
    }
    #endif
