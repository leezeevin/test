...
void eSEED_KeySched(unsigned char *mKey, unsigned int *rKey )
{
...
void eSEED_Encrypt(unsigned int *pOut, unsigned int *pIn, unsigned int *rKey)
{
...
void eSEED_Decrypt(unsigned int *pOut, unsigned int *pIn, unsigned int *rKey)
{
    unsigned int L0, L1, R0, R1;
    unsigned int T0, T1;
...
void eByte2Word(unsigned int *dst, const unsigned char *src, const int srcLen)
{
	int i = 0;
	int remain = 0;
...
void eWord2Byte(unsigned char *dst, const unsigned int *src, const int srcLen)
{
	int i = 0;
	int remain = 0;
...
int SEED_Generate_CMAC(unsigned char *pMAC, int macLen, unsigned char *pIn, int inLen, unsigned char *mKey)
{
	...
	eSEED_KeySched(mKey, rKey);
	eSEED_Encrypt(subKey, subKey, rKey);

	eWord2Byte(L, subKey, BLOCK_SIZE_SEED);
	...
	if (inLen == 0)
	{
		...
		eByte2Word(subKey, L, BLOCK_SIZE_SEED);
		eSEED_Encrypt(temp1, subKey, rKey);
	}
	else
	{
		// make K2
		...
		{
			eWord2Byte(temp, temp1, BLOCK_SIZE_SEED);
			for (j = 0; j < BLOCK_SIZE_SEED; j++)
				temp[j] ^= pIn[BLOCK_SIZE_SEED * i + j];

			eByte2Word(temp1, temp, BLOCK_SIZE_SEED);

			eSEED_Encrypt(temp1, temp1, rKey);
		}
