...
void KISA_SEED_Block_forCTR( unsigned int *in, unsigned int *out, KISA_SEED_KEY *ks )
{
	...
	{
		char* p = (char*) ks->key_data;
		int i=0;

    printf("----> ks : ");
		for ( i=0 ; i<32 ; i++ ) printf("%02X ", (unsigned char)*(p+i));
		printf("\r\n");
...
int SEED_CTR_Process( OUT KISA_SEED_INFO *pInfo, IN DWORD *in, IN int inLen, OUT DWORD *out, OUT int *outLen )
{
	...
	{
    PRINT_BUFFER("KISA_SEED_Block_forCTR pdwCounter", pdwCounter, 32);
		KISA_SEED_Block_forCTR( pdwCounter, out, &pInfo->seed_key );
    PRINT_BUFFER(">>>> in", in, inLen);
    PRINT_BUFFER(">>>> out", out, BLOCK_SIZE_SEED);
		BLOCK_XOR_SEED( out, in, out );
    PRINT_BUFFER(" in <<<<", in, inLen);
    PRINT_BUFFER(" out <<<<", out, BLOCK_SIZE_SEED);

		UpdateCounter_for_SEED( pbszCounter, 1, (BLOCK_SIZE_SEED-1) );
    PRINT_BUFFER("*********** pbszCounter", pbszCounter, BLOCK_SIZE_SEED);
...
void main_kisa_seed_ctr()
{
	...
	ascii_to_hex("828E9E067BC2E9B306A3FA99426787AC", pbszUserKey);
	ascii_to_hex("21C7B0986CF8265708DD9202777B35E7", pbszCounter);
	ascii_to_hex("A1ADCBDDD519B6D40B6258B06CA0C158", InputText);
  ...
  InputCount[0] = nInputTextLen;
    ...
    PRINT_BUFFER("pbszCounter", pbszCounter, 16);
    ...
    PRINT_BUFFER("-----> outbuf", outbuf, nOutLeng+nEncRmainLeng);
  ...
  InputCount[1] = nOutputTextLen;
    ...
    PRINT_BUFFER("SEED_CTR_Process   data", data, process_blockLeng);

    PRINT_BUFFER("SEED_CTR_Process   outbuf", outbuf, process_blockLeng);



		SEED_CTR_Process( &info, data, remainleng, outbuf, &nOutLeng );

    PRINT_BUFFER("data", data, process_blockLeng);

    PRINT_BUFFER("outbuf", outbuf, nOutLeng);
...
void main_kisa_seed_ctr()
{
  ...
  InputCount[0] = nInputTextLen;
    ...
    PRINT_BUFFER(">>>>>>>>>>>>>>>>>>>>> counter", pbszCounter, 16);


		SEED_CTR_init( &info, KISA_ENCRYPT, pbszUserKey, pbszCounter );


    PRINT_BUFFER("*************   pbszUserKey", pbszUserKey, 16);
		printf("outbuf size : %d \r\n", sizeof(unsigned int) * process_blockLeng);
  ...
  InputCount[1] = nOutputTextLen;
    ...
    PRINT_BUFFER("pbszCounter", pbszCounter, 16);
		memset(outbuf, 0, process_blockLeng);
