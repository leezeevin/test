	...
	uint32_t		omgri_len)
{
	...
	qByteLen = (uint32_t)(DIGITSIZE) * (kcdsa->KCDSA_Q->Length); // Q의 바이트단위 길이를 저장 28
	...
	uint32_t		HASH)
{
	...
	ret = BN2OS(BN_Tmp1, i, bzTmp);							GOTO_END; //bzTmp 값 바뀜
	j = i;
	i = 0;
	//printf("\nsign121212\n\n");			//값이 완전 다른 값으로 바뀜
	...
	if (HASH == SHA224)
	{
		KCDSA_SHA224_Init(&SHA224_AlgInfo);	
		KCDSA_SHA224_Update(&SHA224_AlgInfo, bzTmp, j);
		KCDSA_SHA224_Final(&SHA224_AlgInfo, bzTmp); //bzTmp값 바뀜
