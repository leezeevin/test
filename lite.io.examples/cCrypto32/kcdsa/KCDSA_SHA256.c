...
//*********************************************************************************************************************************
// o KCDSA_SHA256_K	: 단계연산에서 사용되는 64개의 상수
//*********************************************************************************************************************************
const uint32_t KCDSA_SHA256_K[64] = {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1,
...
#define FF(a, b, c, d, e, f, g, h, j) {							\
		T1 = h + Sigma1(e) + Ch(e, f, g) + KCDSA_SHA256_K[j] + X[j];	\
		d += T1;												\
...
// o 출력				: 
//*********************************************************************************************************************************
void KCDSA_SHA256_Transform(uint32_t* Message, uint32_t* ChainVar)
{
...
void KCDSA_SHA256_Update(SHA256_ALG_INFO *AlgInfo, uint8_t* Message, uint32_t MessageLen)
{
	...
	if( MessageLen>=PartLen ) {
		...
		KCDSA_SHA256_Transform((DWORD *)AlgInfo->Buffer, AlgInfo->ChainVar);
		...
		while( MessageLen>=SHA256_DIGEST_BLOCKLEN ) {
			if( (((int32_t)Message)%4)==0 ) {	//	Speed up technique
				KCDSA_SHA256_Transform((DWORD *)Message, AlgInfo->ChainVar);
			}
			else {
...
void KCDSA_SHA256_Final(SHA256_ALG_INFO *AlgInfo, uint8_t* Digest)
{
	...
	if (Index > SHA256_DIGEST_BLOCKLEN - 8)
	{
		...
		KCDSA_SHA256_Transform((uint32_t*)AlgInfo->Buffer, AlgInfo->ChainVar);
