...
void Byte2Word(uint32_t* dst, const uint8_t* src, const int32_t srcLen)
{
    ...
    for (i = 0; i < srcLen; i++)
    {
        ...
        if (remain == 0)
            dst[i >> 2] = ((uint32_t)src[i] << 24);
        else if (remain == 1)
            dst[i >> 2] ^= ((uint32_t)src[i] << 16);
        else if (remain == 2)
            dst[i >> 2] ^= ((uint32_t)src[i] << 8);
        else
            dst[i >> 2] ^= ((uint32_t)src[i] & 0x000000FF);
    }
}
...
void Word2Byte(uint8_t* dst, const uint32_t* src, const int32_t srcLen)
{
    ...
    for (i = 0; i < srcLen; i++)
    {
        ...
        if (remain == 0)
            dst[i] = (uint8_t)(src[i >> 2] >> 24);
        else if (remain == 1)
            dst[i] = (uint8_t)(src[i >> 2] >> 16);
        else if (remain == 2)
            dst[i] = (uint8_t)(src[i >> 2] >> 8);
        else
