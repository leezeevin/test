...
#define ECC_int8_t2_FIELD 1 /* c2field */
...
typedef struct _ECC_PARAMS{
	...
	union {
		...
		ECCP_C2FIELD c2field;/* int8_tacteristic-two field */
	};
	MPZ order;
	MPZ cofactor;
} ECC_PARAMS;
...
typedef struct _ECC_PARAMS_TAB{
	SINT field_type; /* ECC_PRIME_FIELD or ECC_int8_t2_FIELD */
...
extern SINT EC_POINT_LENGTH(int32_t Curve_number);

/* ECC Domain Parameters Setting */
SINT ECC_init_params(ECC_FIELD_TYPE	ftype, ECC_PARAMS *ecc_params, const uint32_t ECC_ID);
SINT ECC_set_params(ECC_ID ecc_id,ECC_PARAMS *ecc_params);

SINT Bit_Truncation(unsigned long *IN, const int32_t Truncater_bit_loc);
