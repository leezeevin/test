...
uint8_t int8_t_bit_trancator(uint8_t IN, int32_t bitlen)
{
	if(bitlen == 1)
		return IN & 1;
	else if(bitlen ==2)
...
extern SINT
EC_POINT_LENGTH(int32_t Curve_number)
{
	int32_t length, reduced_length = 0;
	int32_t i=0;
	uint32_t top_word;
	...
	if(ecc_params_tab[Curve_number].field_type == ECC_int8_t2_FIELD)
	{
		while(1)
		{
...
SINT
ECC_init_params(ECC_FIELD_TYPE	ftype, ECC_PARAMS *ecc_params, const uint32_t ECC_ID) // 초기화 
{
	...
	else if (ftype == ECC_int8_t2_FIELD) {
		ecc_params->field_type = ECC_int8_t2_FIELD;
...
ECC_set_params(ECC_ID ecc_id, ECC_PARAMS *ecc_params) // ecc id 는 커브의 분류 번호 , 분류 번호와 선언한 파라미터를 입력하여 파라미터를 세팅하는 함수 // ECC_initialize 이후에 시행해야 멤카피 에러가 안남
{
	...
	}else if(ecc_params->field_type == ECC_int8_t2_FIELD){
...
/*** PRNG ECKCDSA ***/
SINT
Bit_Truncation(unsigned long *IN, const int32_t Truncater_bit_loc)
{
	int32_t Word_t = Truncater_bit_loc / 32;
	int32_t extra_t = Truncater_bit_loc % 32;
...
void PRNG_ECKCDSA_SHA224(const uint8_t *SEED, const SINT SEED_int8_t_LENGTH, uint8_t *output, const SINT output_bit_LENGTH)  // output should be initialized
{
	int32_t k,r;   // refer to specification of ECKCDSA
	int32_t output_byte_length, Byte_remainder_bit;
	int32_t Byte_remainder_length=0, Word_remainder_length=0;
	int32_t index, j, extra_index=0;
	SHA224_ALG_INFO	AlgInfo;
	const int32_t SHA224_DIGEST_BYTE_LENGTH = 28;
	uint8_t DigestValue[28];
	uint8_t temp_out[200]={0,};
	uint8_t SEED_i[200]={0,};
	uint8_t Toutput[200]={0,};
	...
	for(index=0;index<SEED_int8_t_LENGTH;index++) 
	...
	for(index=0;index<k+1;index++)
	{
		...
		{
			SEED_i[SEED_int8_t_LENGTH] = index;

			/* SEED 블록에 대한 해쉬 연산 */
			EC_KCDSA_SHA224_Init(&AlgInfo);
			EC_KCDSA_SHA224_Update(&AlgInfo, SEED_i, SEED_int8_t_LENGTH+1);
			EC_KCDSA_SHA224_Final(&AlgInfo, DigestValue);
			...
			if(r!=0)
			{
				...
				Toutput[0] = int8_t_bit_trancator(Toutput[0], Byte_remainder_bit);
			}
			else
			{
...
void PRNG_ECKCDSA_SHA256(const uint8_t *SEED, const SINT SEED_int8_t_LENGTH, uint8_t *output, const SINT output_bit_LENGTH)  // output should be initialized
{
	int32_t k,r;   // refer to specification of ECKCDSA
	int32_t output_byte_length, Byte_remainder_bit;
	int32_t Byte_remainder_length=0, Word_remainder_length=0;
	...
	const int32_t SHA256_DIGEST_BYTE_LENGTH = 32;
	uint8_t DigestValue[32];
	uint8_t temp_out[200]={0,};
	uint8_t SEED_i[200]={0,};
	uint8_t Toutput[200]={0,};
	...
	for(index=0;index<k+1;index++)
	{
		if((index==k && r!=0) || ((index==(k-1)) && r==0))  // r!=0 이면 마지막 블록은 k 이다. r=0 이면 마지막 블록은 k-1 이다. 
		{
			...
			/* SEED 블록에 대한 해쉬 연산 */
			EC_KCDSA_SHA256_Init(&AlgInfo);
			EC_KCDSA_SHA256_Update(&AlgInfo, SEED_i, SEED_int8_t_LENGTH+1);
			EC_KCDSA_SHA256_Final(&AlgInfo, DigestValue);
