...
//*********************************************************************************************************************************
// o EC_KCDSA_SHA256_K	: 단계연산에서 사용되는 64개의 상수
//*********************************************************************************************************************************
const uint32_t EC_KCDSA_SHA256_K[64] = {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1,
...
#define FF(a, b, c, d, e, f, g, h, j) {							\
		T1 = h + Sigma1(e) + Ch(e, f, g) + EC_KCDSA_SHA256_K[j] + X[j];	\
		d += T1;												\
		h = T1 + Sigma0(a) + Maj(a, b, c);						\
}
		
//*********************************************************************************************************************************
// o EC_KCDSA_SHA256_Transform()	: 512 비트 단위 블록의 메시지를 입력 받아 연쇄변수를 갱신하는 압축 함수로써
//						  4 라운드(64 단계)로 구성되며 8개의 연쇄변수(a, b, c, d, e, f, g, h)를 사용
// o 입력				: Message		- 입력 메시지의 포인터 변수
//						  ChainVar		- 연쇄변수의 포인터 변수
// o 출력				: 
//*********************************************************************************************************************************
void EC_KCDSA_SHA256_Transform(uint32_t* Message, uint32_t* ChainVar)
{
	uint32_t a, b, c, d, e, f, g, h, T1, X[64];
	uint32_t j;
...
void EC_KCDSA_SHA256_Update(SHA256_ALG_INFO *AlgInfo, uint8_t* Message, uint32_t MessageLen)
{
	...
	if( MessageLen>=PartLen ) {
		memcpy(AlgInfo->Buffer+RemainedLen, Message, (int32_t)PartLen);
		EC_KCDSA_SHA256_Transform((DWORD *)AlgInfo->Buffer, AlgInfo->ChainVar);
		...
		while( MessageLen>=SHA256_DIGEST_BLOCKLEN ) {
			if( (((int32_t)Message)%4)==0 ) {	//	Speed up technique
				EC_KCDSA_SHA256_Transform((DWORD *)Message, AlgInfo->ChainVar);
			}
			else {
				memcpy((BYTE *)AlgInfo->Buffer, Message, (int32_t)SHA256_DIGEST_BLOCKLEN);
				EC_KCDSA_SHA256_Transform((DWORD *)AlgInfo->Buffer, AlgInfo->ChainVar);
			}
	...
	memcpy((BYTE *)AlgInfo->Buffer+RemainedLen, Message, (int32_t)MessageLen);
}
...
{
	uint32_t i, Index, CountL, CountH;
	...
	{
		memset((uint8_t*)AlgInfo->Buffer + Index, 0, (int32_t)(SHA256_DIGEST_BLOCKLEN - Index));
		EC_KCDSA_SHA256_Transform((uint32_t*)AlgInfo->Buffer, AlgInfo->ChainVar);
		memset((uint8_t*)AlgInfo->Buffer, 0, (int32_t)SHA256_DIGEST_BLOCKLEN - 8);
	}
	else
		memset((uint8_t*)AlgInfo->Buffer + Index, 0, (int32_t)(SHA256_DIGEST_BLOCKLEN - Index - 8));
	...
	// 메시지 길이 덧붙이기
	((uint32_t*)AlgInfo->Buffer)[SHA256_DIGEST_BLOCKLEN / 4 - 2] = CountH;
	((uint32_t*)AlgInfo->Buffer)[SHA256_DIGEST_BLOCKLEN / 4 - 1] = CountL;
