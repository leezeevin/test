#include <stdio.h>
#include <stdlib.h>
#include "ec-kcdsa.h"
#include "__util.h"

void ec_kcdsa_main() {
	MPZ sk;
	ECC_PARAMS curve;
	ECC_POINT public_point;

	uint8_t msg[] = 
	{ 
		0x54, 0x68, 0x69, 0x73, 0x20, 0x69, 0x73, 0x20, 0x61, 0x20, 0x73, 0x61, 0x6D, 0x70, 0x6C, 0x65,
		0x20, 0x6D, 0x65, 0x73, 0x73, 0x61, 0x67, 0x65, 0x20, 0x66, 0x6F, 0x72, 0x20, 0x45, 0x43, 0x2D,
	    0x4B, 0x43, 0x44, 0x53, 0x41, 0x20, 0x69, 0x6D, 0x70, 0x6C, 0x65, 0x6D, 0x65, 0x6E, 0x74, 0x61,
	    0x74, 0x69, 0x6F, 0x6E, 0x20, 0x76, 0x61, 0x6C, 0x69, 0x64, 0x61, 0x74, 0x69, 0x6F, 0x6E, 0x2E 
	};
	int32_t msgLen = 64;
	uint8_t user_provided_random_input[] =
	{ 
		 
	};

	uint8_t sm[] = "F428024B3D689F182C21DDB977214E30E1F901849A9D348574FB4D33";
	uint8_t x_Q[] = "C456F7EB5FAEB08F5FC5C6E1BFA0858DAC0828B722A6A475B1E1C518";
	uint8_t y_Q[] = "3C74BB1DFD16D789A98A78993531F512FA3D115B4F908A6D2448DCAE";

	int32_t user_provided_random_input_Len = 160;
	uint8_t sig[72];

	int32_t x_Q_len = (sizeof(x_Q) - 1) / 8;
	int32_t y_Q_len = (sizeof(y_Q) - 1) / 8;
	int32_t mSize = 0;

	uint8_t m[(sizeof(sm) - 1) / 2] = { 0, };
	uint32_t* x = calloc(x_Q_len, sizeof(uint32_t));
	uint32_t* y = calloc(y_Q_len, sizeof(uint32_t));

	bigNumToArray(x_Q, (sizeof(x_Q) - 1), x);
	bigNumToArray(y_Q, (sizeof(y_Q) - 1), y);
	toHex(sm, m, &mSize);

	ECC_init_params(ECC_PRIME_FIELD, &curve, SECP224r);

	ECC_set_params(SECP224r, &curve);

	Private_Key_generator(&sk, curve, SECP224r, SHA224, user_provided_random_input, user_provided_random_input_Len);

	//Public_Key_generator(&public_point, sk.dat, curve);
	Public_Key_set_params(&public_point, curve, x, x_Q_len, y, y_Q_len);

	printf(" Qx\n");
	for (int i = 0; i < public_point.pfield_point.x.len; i++)
	{
			//if (i % 4 == 0 && i != 0) printf("\n");
			printf("%08X", public_point.pfield_point.x.dat[i]);
	}
	printf("\n\nQy \n");
	for (int i = 0; i < public_point.pfield_point.y.len; i++)
	{
			//if (i % 4 == 0 && i != 0) printf("\n");
			printf("%08X", public_point.pfield_point.y.dat[i]);
	}
	
	//EC_KCDSA_sign(public_point, curve, sk.dat, SECP224r, SHA224, m, mSize, sig);

	uint8_t sr[] = "88F33985A29B5E43EFDF645C3F9811A11677B89F3AFB74E536D79AEE";
	uint8_t ss[] = "633DA85D219A8C998C09F195025DE4B9C78E6931320BE88B7FE67503";
	uint32_t siglen = 0;

  toHex(&sr, &sig, &siglen);
	toHex(&ss, &sig, &siglen);

	printf("\n\nR = ");
	for (int32_t i = 0; i < 28; i++) {
		printf("%02X ", sig[i]);
	}
	printf("\n");
	printf("S = ");
	for (int32_t i = 28; i < 56; i++) {
		printf("%02X ", sig[i]);
	}
	printf("\n");

	int32_t ret = EC_KCDSA_verify(public_point, sig, curve, SECP224r, SHA224, m, mSize);
	printf("verify :");
	if (ret == 0)
		printf("Success\n");
	else
		printf("Fail\n");
}
