...
void Point_to_Byte(uint32_t *W, uint8_t *C, int32_t W_length, const int32_t Temp_int8_t_length)
{
	...
	for (i = 0; i < Temp_int8_t_length; i++)
	{
		C[i] = TEMP[Temp_int8_t_length - i - 1];
	}

}

void
eckcdsa_gfp(const ECC_POINT Private, const ECC_PARAMS SELECTED_CURVE, uint32_t *secret, const uint32_t ECC_ID, 
	const uint32_t HASH_FUNCTION, uint8_t Message[], const int32_t MSG_len, uint8_t signature[])
{
	int32_t i, added_len = 0, added_len2 = 0;
	uint32_t t, u, u2, K_Word_length;
	SHA224_ALG_INFO AlgInfo_224;
	SHA256_ALG_INFO AlgInfo_256;
	uint8_t KKEY_VAL_[32];

	uint8_t Hash_out_r[MAX_GF_BUF_LEN], Hash_out_v[MAX_GF_BUF_LEN];
	ECC_POINT kG;
	ECC_POINT Q;
	...
	uint8_t r_xor_v_dat[MAX_GF_BUF_LEN] = { 0, };

	uint32_t d_inv_dat[MAX_GF_BUF_LEN];
	uint8_t int8_t_cQ_x[100] = { 0, }, int8_t_cQ_y[100] = { 0, }, c_Q_M[200] = { 0, }, int8_t_kG_x[100] = { 0, }, Temp_int8_t_length;
	int32_t count; // HASH_OUTPUT_LENGTH와 Temp_int8_t_length의 차이로 발생하는 r,v 문제를 해결하기 위함
	uint32_t KVAL_dat[100] = { 0, }, K_dat[100] = { 0, };
	uint8_t KVAL_int8_t_dat[500] = { 0, };
	MPZ K, KVAL;
	MPZ d_inv, d;
	MPZ e, s, r_xor_v, k_e;

	int32_t HASH_INPUT_LENGTH = EC_KCDSA_HAS160_INPUT_BYTE_LENGTH, HASH_OUTPUT_LENGTH;
	int32_t KKEY_VAL_len;
	...
	if (HASH_FUNCTION == SHA224)
		PRNG_ECKCDSA_SHA224(KKEY_VAL_, KKEY_VAL_len, KVAL_int8_t_dat, u2);
	else if (HASH_FUNCTION == SHA256)
		PRNG_ECKCDSA_SHA256(KKEY_VAL_, KKEY_VAL_len, KVAL_int8_t_dat, u2);
	else
		return;
	
	// k = SNGF(KVAL, 2u) mod n 계산
	Byte_to_Prime_Field(KVAL_int8_t_dat, KVAL_dat, u2 / 8 + added_len2, K_Word_length * 2);
	...
	// r = HASH(x_1) 계산
	Temp_int8_t_length = EC_POINT_LENGTH(ECC_ID);

	Point_to_Byte(kG.pfield_point.x.dat, int8_t_kG_x, kG.pfield_point.x.len, Temp_int8_t_length);

	if (HASH_FUNCTION == SHA224)
	{
		EC_KCDSA_SHA224_Init(&AlgInfo_224);
		EC_KCDSA_SHA224_Update(&AlgInfo_224, int8_t_kG_x, Temp_int8_t_length);
		EC_KCDSA_SHA224_Final(&AlgInfo_224, Hash_out_r);
		memcpy(signature, Hash_out_r, HASH_OUTPUT_LENGTH);
	}
	else if (HASH_FUNCTION == SHA256)
	{
		EC_KCDSA_SHA256_Init(&AlgInfo_256);
		EC_KCDSA_SHA256_Update(&AlgInfo_256, int8_t_kG_x, Temp_int8_t_length);
		EC_KCDSA_SHA256_Final(&AlgInfo_256, Hash_out_r);
	...
	Point_to_Byte(Q.pfield_point.x.dat, int8_t_cQ_x, Q.pfield_point.x.len, Temp_int8_t_length);
	Point_to_Byte(Q.pfield_point.y.dat, int8_t_cQ_y, Q.pfield_point.x.len, Temp_int8_t_length);

	for (i = 0; i < Temp_int8_t_length; i++)
	{
		c_Q_M[i] = int8_t_cQ_x[i];
		c_Q_M[i + Temp_int8_t_length] = int8_t_cQ_y[i];
	}
	for (i = Temp_int8_t_length * 2; i < HASH_INPUT_LENGTH; i++)	c_Q_M[i] = 0;
	...
	{
		EC_KCDSA_SHA224_Init(&AlgInfo_224);
		EC_KCDSA_SHA224_Update(&AlgInfo_224, c_Q_M, HASH_INPUT_LENGTH + MSG_len);
		EC_KCDSA_SHA224_Final(&AlgInfo_224, Hash_out_v);
	}
	else if (HASH_FUNCTION == SHA256)
	{
		EC_KCDSA_SHA256_Init(&AlgInfo_256);
		EC_KCDSA_SHA256_Update(&AlgInfo_256, c_Q_M, HASH_INPUT_LENGTH + MSG_len);
		EC_KCDSA_SHA256_Final(&AlgInfo_256, Hash_out_v);
	}

	// (r, v) mod 2 ^ 8w 계산
	count = HASH_OUTPUT_LENGTH - Temp_int8_t_length;
	if (count > 0) {
		for (i = 0; i < count; i++) {
			Hash_out_r[i] = 0x00;
			Hash_out_v[i] = 0x00;
	...
	Point_to_Byte(s.dat, signature + KKEY_VAL_len, s.len, Temp_int8_t_length);
}

void
eckcdsa_gf2n(const ECC_POINT Private, const ECC_PARAMS SELECTED_CURVE, uint32_t *secret, const uint32_t ECC_ID,
	const uint32_t HASH_FUNCTION, uint8_t Message[], const int32_t MSG_len, uint8_t signature[])
{
	int32_t i, added_len = 0, added_len2 = 0;
	uint32_t t, u, u2, K_Word_length;
	...
	uint8_t r_xor_v_int8_t_dat[MAX_GF_BUF_LEN] = { 0, };

	uint32_t r_xor_v_dat[MAX_GF_BUF_LEN], KVAL_dat[100] = { 0, }, K_dat[100] = { 0, };
	uint8_t int8_t_cQ_x[100] = { 0, }, int8_t_cQ_y[100] = { 0, }, c_Q_M[200] = { 0, }, int8_t_kG_x[100] = { 0, }, Temp_int8_t_length = 0;
	...
	Point_to_Byte(kG.c2field_point.x, int8_t_kG_x, SELECTED_CURVE.order.len, Nbyte);

	if (HASH_FUNCTION == SHA224)
	{
		EC_KCDSA_SHA224_Init(&AlgInfo_224);
		EC_KCDSA_SHA224_Update(&AlgInfo_224, int8_t_kG_x, Nbyte);
		EC_KCDSA_SHA224_Final(&AlgInfo_224, Hash_out_r);
		memcpy(signature, Hash_out_r, HASH_OUTPUT_LENGTH);
	}
	else if (HASH_FUNCTION == SHA256)
	{
		EC_KCDSA_SHA256_Init(&AlgInfo_256);
		EC_KCDSA_SHA256_Update(&AlgInfo_256, int8_t_kG_x, Nbyte);
		EC_KCDSA_SHA256_Final(&AlgInfo_256, Hash_out_r);
		if(N == 233 || N == 232)
			memcpy(signature, Hash_out_r + (HASH_OUTPUT_LENGTH - KKEY_VAL_len), KKEY_VAL_len);
		else
	...
	Point_to_Byte(Q.c2field_point.x, int8_t_cQ_x, SELECTED_CURVE.order.len, Nbyte);
	Point_to_Byte(Q.c2field_point.y, int8_t_cQ_y, SELECTED_CURVE.order.len, Nbyte);

	for (i = 0; i < Nbyte; i++)		c_Q_M[i] = int8_t_cQ_x[i];
	for (i = Nbyte; i < Nbyte * 2; i++)		c_Q_M[i] = int8_t_cQ_y[i - Nbyte];
	...
	// e = r ^ v mod n 계산
	for (i = 0; i < HASH_OUTPUT_LENGTH; i++)	r_xor_v_int8_t_dat[i] = Hash_out_r[i] ^ Hash_out_v[i];

	MAC_INIT_MPZ(r_xor_v, r_xor_v_dat, MPCONST_POS_SIG);
	r_xor_v.len = HASH_OUTPUT_LENGTH / 4;

	Byte_to_Prime_Field(r_xor_v_int8_t_dat, r_xor_v.dat, HASH_OUTPUT_LENGTH, r_xor_v.len);
	...
	Point_to_Byte(s.dat, signature + Temp_int8_t_length, s.len, Temp_int8_t_length);
}
	...
	const uint32_t ECC_ID, const uint32_t HASH_FUNCTION, uint8_t Message[], const int32_t MSG_len)
{
	...
	uint8_t r_xor_v_dat[MAX_GF_BUF_LEN] = { 0, };
	uint8_t rr_dat[100] = { 0x00, };
	uint32_t ss_dat[100] = { 0x00, };

	uint8_t int8_t_cQ_x[100] = { 0, }, int8_t_cQ_y[100] = { 0, }, c_Q_M[200] = { 0, }, int8_t_kG_x[100] = { 0, }, int8_t_sQ_eG[100] = { 0, };
	int32_t Temp_int8_t_length;
	...
	// (x_2, y_2) = t'Q + e'G
	MAC_INIT_MPZ(s, s_dat, MPCONST_POS_SIG);
	Byte_to_Prime_Field(signature + Temp_int8_t_length, ss_dat, Temp_int8_t_length, SELECTED_CURVE.order.len);
	...
	Point_to_Byte(sQ_eG.pfield_point.x.dat, int8_t_sQ_eG, SELECTED_CURVE.order.len, Temp_int8_t_length);

	// Hash(x_2')
	if(HASH_FUNCTION == SHA224)
	{
		EC_KCDSA_SHA224_Init(&AlgInfo_224);
		EC_KCDSA_SHA224_Update(&AlgInfo_224, int8_t_sQ_eG, Temp_int8_t_length);
		EC_KCDSA_SHA224_Final(&AlgInfo_224, Hash_out_r);
	}
	else if (HASH_FUNCTION == SHA256)
	{
		EC_KCDSA_SHA256_Init(&AlgInfo_256);
		EC_KCDSA_SHA256_Update(&AlgInfo_256, int8_t_sQ_eG, Temp_int8_t_length);
		EC_KCDSA_SHA256_Final(&AlgInfo_256, Hash_out_r);
	}
...
{
	int32_t count;
	int32_t N = 0, Nbyte = 0, added_len = 0;
	...
	uint32_t r_xor_v_dat[MAX_GF_BUF_LEN];
	uint8_t int8_t_cQ_x[100] = { 0, }, int8_t_cQ_y[100] = { 0, }, c_Q_M[200] = { 0, }, int8_t_kG_x[100] = { 0, }, int8_t_sQ_eG[100] = { 0, };
	int32_t Temp_int8_t_length = 0;
	uint32_t v_c = 1;
	...
	// e' = r' ^ v' mod n 계산
	for (i = 0; i < HASH_OUTPUT_LENGTH; i++)	r_xor_v_int8_t_dat[i] = rr_dat[i] ^ Hash_out_v[i];
	...
	Point_to_Byte(sQ_eG.c2field_point.x, int8_t_sQ_eG, SELECTED_CURVE.order.len, Temp_int8_t_length);
	
	// Hash(x_2')
	if (HASH_FUNCTION == SHA224)
	{
		EC_KCDSA_SHA224_Init(&AlgInfo_224);
		EC_KCDSA_SHA224_Update(&AlgInfo_224, int8_t_sQ_eG, Nbyte);
		EC_KCDSA_SHA224_Final(&AlgInfo_224, Hash_out_r);
	}
	else if (HASH_FUNCTION == SHA256)
	{
		EC_KCDSA_SHA256_Init(&AlgInfo_256);
		EC_KCDSA_SHA256_Update(&AlgInfo_256, int8_t_sQ_eG, Nbyte);
		EC_KCDSA_SHA256_Final(&AlgInfo_256, Hash_out_r);
	}
...
{
	uint32_t tmp_XKEY[9] = { 0x00, };
	uint32_t XSEED_dat[9] = { 0x00, }, final_KEY_dat[100] = { 0, };
	uint32_t XVAL_dat[9] = { 0x00, }, G_XVAL_2u_dat[100] = { 0, };
	MPZ XKEY, XSEED, XVAL;
	MPZ final_KEY, G_XVAL_2u;
	int32_t added_len = 0, added_len2 = 0, i;

	uint8_t G_XVAL_2u_int8_t_dat[500] = { 0, };
	uint8_t XVAL_int8_t_dat[36] = { 0x00, };
	uint8_t XSEED_int8_t_dat[36] = { 0x00, };

	int32_t t, u, u2, finalkey_Word_length;
	...
	if (HASH_FUNCTION == SHA224)
		PRNG_ECKCDSA_SHA224(URAND_VAL, URAND_len, XSEED_int8_t_dat, t);
	else if (HASH_FUNCTION == SHA256)
		PRNG_ECKCDSA_SHA256(URAND_VAL, URAND_len, XSEED_int8_t_dat, t);
	else
		return;

	Byte_to_Prime_Field(XSEED_int8_t_dat, XSEED_dat, t / 8 + added_len, finalkey_Word_length);
	...
	Word_to_Byte(XVAL.dat, XVAL_int8_t_dat, finalkey_Word_length, t / 8 + added_len);

	// SNGF(XVAL, 2u) 생성
	if (HASH_FUNCTION == SHA224)
		PRNG_ECKCDSA_SHA224(XVAL_int8_t_dat, t / 8 + added_len, G_XVAL_2u_int8_t_dat, u2);
	else if (HASH_FUNCTION == SHA256)
		PRNG_ECKCDSA_SHA256(XVAL_int8_t_dat, t / 8 + added_len, G_XVAL_2u_int8_t_dat, u2);
	else
		return;

	Byte_to_Prime_Field(G_XVAL_2u_int8_t_dat, G_XVAL_2u_dat, u2 / 8 + added_len2, finalkey_Word_length * 2);
...
{
	int32_t a;
	uint32_t d_inv_dat[MAX_GF_BUF_LEN] = { 0, };

	MPZ d_inv, d;

	Public_point->pfield_point.x.dat = (uint32_t*)malloc(MAX_GF_BUF_LEN * sizeof(uint32_t));
	Public_point->pfield_point.y.dat = (uint32_t*)malloc(MAX_GF_BUF_LEN * sizeof(uint32_t));
...
void Public_Key_generator_gf2n(ECC_POINT *Public_point, uint32_t *Private_key, ECC_PARAMS SELECTED_CURVE)
{
	...
	Public_point->c2field_point.x = (uint32_t*)malloc(MAX_GF_BUF_LEN * sizeof(uint32_t));
	Public_point->c2field_point.y = (uint32_t*)malloc(MAX_GF_BUF_LEN * sizeof(uint32_t));
...
void Public_Key_generator(ECC_POINT *Public_point, uint32_t *Private_key, ECC_PARAMS SELECTED_CURVE) {
	...
	else if (SELECTED_CURVE.field_type == ECC_int8_t2_FIELD) {
...
void Public_Key_set_params(ECC_POINT *Public_point, const ECC_PARAMS SELECTED_CURVE, const uint32_t *x_Q, int32_t x_Q_len, const uint32_t *y_Q, int32_t y_Q_len)
{
	if (SELECTED_CURVE.field_type == ECC_PRIME_FIELD) {
		...
		for (int32_t i = 0; i < x_Q_len; ++i) Public_point->pfield_point.x.dat[i] = x_Q[i];
		for (int32_t i = 0; i < y_Q_len; ++i) Public_point->pfield_point.y.dat[i] = y_Q[i];
	}
	else if (SELECTED_CURVE.field_type == ECC_int8_t2_FIELD) {
		Public_point->c2field_point.x = (uint32_t*)malloc(MAX_GF_BUF_LEN * sizeof(uint32_t));
		Public_point->c2field_point.y = (uint32_t*)malloc(MAX_GF_BUF_LEN * sizeof(uint32_t));
		...
		for (int32_t i = 0; i < x_Q_len; ++i) Public_point->c2field_point.x[i] = x_Q[i];
		for (int32_t i = 0; i < y_Q_len; ++i) Public_point->c2field_point.y[i] = y_Q[i];
	}
	else
		return;
}
