...
int32_t __nwebsocket_open(cnwebsocket* p)
{
  ...
  sprintf(_json, 
          "{"
            ...
            "\"TIMEOUT\":\"0\","
          "}",
            p->io.argv[WEBSOCKET_URL],
            p->io.argv[WEBSOCKET_CSTYPE],
  ...
  for ( e=0 ; e<MAX_CLIENT ; e++ )
  {
    ...
    (p->buf+e)->_sz = atoi(p->io.argv[SIZE_OF_BUFFER]);
  }

  p->io.fd = e = __websocket_open(&p->io.h, _json, p->io.buf, atoi(p->io.argv[SIZE_OF_BUFFER]), f, p);
  return e;
}
...
int32_t __nwebsocket_close_fd(cnwebsocket* p, int32_t fd)
{
  int32_t e = 0;
  if ( p->io.h==0 ) return 0xE0000001;
  e = __websocket_close_fd(&p->io.h,fd);
