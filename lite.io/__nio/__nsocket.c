...
int32_t __nsocket_status(void* h, int32_t fd, int8_t* b, int32_t sz, void* extra, void* o)
{
  ...
  switch(sz)
  {
    ...
    case 0xE000FD0A:
      /////// Connected
      e = p->io.__f[__CALLBACK_CONNECTED]?p->io.__f[__CALLBACK_CONNECTED](h, fd, b, sz, extra, o):0;
      break;

    case 0xE000FD0F:
      /////// DisConnected
      e = p->io.__f[__CALLBACK_DISCONNECTED]?p->io.__f[__CALLBACK_DISCONNECTED](h, fd, b, sz, extra, o):0;
      break;
  }
...
int32_t __nsocket_open(cnsocket* p)
{
  ...
  sprintf(_json, 
          ...
          "}",
            ...
            p->io.argv[SOCKET_CSTYPE][0]=='C'?"4000000":"0"
            );
  e = __socket_open(&p->io.h, _json, p->io.buf, 4096, f, p);
  return e;
}
...
int32_t __nsocket_close_fd(cnsocket* p, int32_t fd)
{
  int32_t e = 0;
  if ( p->io.h==0 ) return 0xE0000001;
  e = __socket_close_fd(&p->io.h, fd);
